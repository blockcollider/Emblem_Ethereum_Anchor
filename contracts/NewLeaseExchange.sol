...
contract NewLeaseExchange is ReentrancyGuard, Ownable {
  ...
  function _placeOrder(address maker, uint256 price, uint256 amount, bool demand, uint256 duration, bytes12 vanity) internal returns (bool){
    ...
    if(demand) {
      ...
      if (
        ...
        (EMB.allowance(maker, this) - LEMB.getAmountForUserMining(maker) < amount)
      ) return (false);
    }

    uint256 id = getNewId();

    orders[id] = Order(id, maker,vanity, amount, price, demand, duration,now);
    allOrders.push(id);
    ...
    emit OrderPlaced(id, maker, amount, price,demand, duration,now);
  ...
  function _takeOrder(address taker, uint256 id, uint256 amount,bytes12 vanity) internal returns (bool) {
    ...
    if(order.demand){
      ...
      EMB.transferFrom(taker, this, amount - fee_maker);
    ...
    emit OrderFilled(id, order.maker, taker, order.amount, amount, order.price, order.duration, order.demand, lembId, now);
  ...
  function takeOrders(uint256 []ids,uint256[]amounts, uint256 price, uint256 amount, bool demand, uint256 duration, bytes12 vanity) external payable weiSendGuard nonReentrant returns (bool) {
  ...
  function startMining(uint256 id) public returns (bool) {
    ...
    require((EMB.allowance(msg.sender, this) + LEMB.getAmountForUserMining(msg.sender) >= LEMB.getAmount(id)));
